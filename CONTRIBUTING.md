...
4. **分支** — 基于 `main` 创建分支：
   ...
   # 例如：git checkout -b fix/singbox-dns-解析
   ```
5. **修复与测试** — 实现修复并添加覆盖该缺陷的回归测试。
6. **PR** — 提交 Pull Request。PR 描述中**必须**包含 `Fixes #123` 来关联 Issue（合并后会自动关闭）。
7. **审查** — 维护者审查修复。
8. **合并** — 修复合入 `main`。
9. **发布** — 修复随下一个补丁版本（如 `0.3.0` → `0.3.1`）发布。
...
```
功能建议 → 讨论 → 功能分支 → PR → 审查 → 合并 → 次版本发布
```

1. **建议** — 先提交功能建议 Issue 讨论方案。
2. **共识** — 维护者确认方向并打上 `enhancement` 标签后再开始编码。
3. **分支** — 基于 `main` 创建分支：
   ```bash
   git checkout -b feature/简短描述
   # 例如：git checkout -b feature/hysteria2-协议
   ```
4. **实现** — 编写代码、测试和文档（新增协议的 PR 要求见下方）。
5. **PR** — 提交 Pull Request 并关联功能建议 Issue。
6. **审查** — 维护者审查代码。
7. **合并** — 功能合入 `main`。
8. **发布** — 功能随下一个次版本（如 `0.2.0` → `0.3.0`）发布。
