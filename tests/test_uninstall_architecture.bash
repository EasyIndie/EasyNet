...
assert_equals "subscription-exposure" "$(resolve_uninstall_modules 8)" "Uninstall menu 8 resolves to independent subscription exposure"
assert_equals "__exit__" "$(resolve_uninstall_modules 9)" "Uninstall menu 9 resolves to exit sentinel"

all_uninstall_modules="$(resolve_uninstall_modules 0 | xargs)"
assert_equals "xray-reality hysteria2 trojan-go v2ray shadowsocks wireguard nginx-exposure subscription-exposure" "$all_uninstall_modules" "Uninstall menu 0 removes all protocol modules and exposure layers"
